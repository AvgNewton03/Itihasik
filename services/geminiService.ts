...
const API_KEY = (() => {
  ...
  if (typeof console !== "undefined") {
    console.log(
      "[Gemini Service] Initialized. Key Present:",
      key ? "Yes (Starts with " + key.substring(0, 4) + ")" : "No"
    );
  }
  ...
  if (!key && typeof console !== "undefined")
    console.warn(
      "Gemini API key not set. Add `VITE_GEMINI_API_KEY` to `.env.local` and restart the dev server."
    );
  return key || "";
})();
...
const FALLBACK_TEMPLES = [
  {
    ...
    description:
      "The 13th-century Sun Temple at Konark, designed as a colossal chariot with 24 wheels, dedicated to the Sun God Surya.",
    imageSearchTerm: "Konark Sun Temple stone wheels architecture",
    tags: ["Odisha", "Sun God", "Chariot"],
    lat: 19.8876,
    lng: 86.0945,
  },
  {
    ...
    description:
      "A masterpiece of Chola architecture in Thanjavur, featuring one of the tallest temple towers in the world.",
    imageSearchTerm: "Brihadeeswarar Temple Thanjavur vimana",
    tags: ["Tamil Nadu", "Chola", "Shiva"],
    lat: 10.7828,
    lng: 79.1318,
  },
  {
    ...
    description:
      "Located in the Himalayas, this ancient Shiva temple is one of the Chota Char Dham pilgrimage sites.",
    imageSearchTerm: "Kedarnath Temple himalayas snow stone",
    tags: ["Himalayas", "Shiva", "Jyotirlinga"],
    lat: 30.7352,
    lng: 79.0669,
  },
  {
    ...
    imageSearchTerm: "Meenakshi Amman Temple colorful gopuram",
    tags: ["Madurai", "Dravidian", "Parvati"],
    lat: 9.9195,
    lng: 78.1193,
  },
  {
    ...
    description:
      "Famous for their nagara-style architectural symbolism and their erotic sculptures.",
    imageSearchTerm: "Khajuraho temple sculptures detailed",
    tags: ["Madhya Pradesh", "Chandela", "Art"],
    lat: 24.8318,
    lng: 79.9199,
  },
  {
    ...
    title: "Golden Temple",
    description:
      "The holiest Gurdwara of Sikhism, located in the city of Amritsar, Punjab, India.",
    imageSearchTerm: "Golden Temple Amritsar night reflection",
    tags: ["Punjab", "Sikhism", "Gold"],
    lat: 31.62,
    lng: 74.8765,
  },
];
...
const FALLBACK_GODS = [
  {
    ...
    title: "Shiva",
    description:
      "The Destroyer within the Trimurti, the Hindu trinity that includes Brahma and Vishnu.",
    imageSearchTerm: "Lord Shiva meditation himalayas",
    tags: ["Trimurti", "Destroyer", "Yogi"],
    lat: 30.7352,
    lng: 79.0669,
  },
  {
    ...
    title: "Vishnu",
    description:
      "The Preserver who descends to earth as avatars to restore cosmic order.",
    imageSearchTerm: "Lord Vishnu cosmic ocean serpent",
    tags: ["Preserver", "Dashavatara", "Cosmic"],
    lat: 0,
    lng: 0,
  },
  {
    ...
    title: "Ganesha",
    description:
      "The remover of obstacles, patron of arts and sciences, and the deva of intellect and wisdom.",
    imageSearchTerm: "Ganesha elephant god intricate art",
    tags: ["Wisdom", "Beginnings", "Elephant"],
    lat: 19.169,
    lng: 73.0085,
  },
  {
    ...
    title: "Durga",
    description:
      "A major form of the Hindu goddess Parvati, associated with protection, strength, motherhood.",
    imageSearchTerm: "Goddess Durga lion trishul fierce",
    tags: ["Shakti", "Warrior", "Mother"],
    lat: 22.5726,
    lng: 88.3639,
  },
];
...
const FALLBACK_TEXTS = [
  {
    ...
    imageSearchTerm: "Bhagavad Gita Krishna Arjuna chariot war",
    tags: ["Philosophy", "Mahabharata", "Krishna"],
    lat: 29.9695,
    lng: 76.8783,
  },
  {
    ...
    title: "The Rigveda",
    description:
      "The oldest known Vedic Sanskrit text, a collection of over 1,000 hymns.",
    imageSearchTerm: "Rigveda ancient palm leaf manuscript sanskrit",
    tags: ["Vedas", "Hymns", "Sanskrit"],
    lat: 0,
    lng: 0,
  },
  {
    ...
    title: "Arthashastra",
    description:
      "An ancient Indian Sanskrit treatise on statecraft, economic policy and military strategy.",
    imageSearchTerm: "Chanakya writing arthashastra ancient",
    tags: ["Politics", "Chanakya", "Economics"],
    lat: 25.6126,
    lng: 85.1588,
  },
];
...
export const generateHistoryResponse = async (
  ...
  if (!isGeminiConfigured())
    return {
      text: "API Key is missing. Please configure it to chat with the historian.",
      imagePrompt: "",
    };
  ...
  try {
    ...
    return {
      text: cleanText,
      imagePrompt:
        parts.length > 1 ? parts[1].trim() : `${prompt} ancient india`,
    };
  } catch (error) {
...
export const getTopicLocation = async (
  topic: string
): Promise<
  | { name: string; googleMapsUri: string; lat?: number; lng?: number }
  | undefined
> => {
  const knownLocation = [
    ...FALLBACK_TEMPLES,
    ...
    (i) =>
      ...
      topic.toLowerCase().includes(i.title.toLowerCase())
  );
...
export const generateTopicDetails = async (
  ...
  const getFallback = () => {
    const fallbackItem = [
      ...
      (i) =>
        i.title === topic || topic.includes(i.title) || i.title.includes(topic)
    );
    if (fallbackItem) {
      return {
        ...
        facts: [
          "Record from local archives.",
          "Full details pending restoration.",
        ],
        galleryPrompts: [
          fallbackItem.title + " architecture",
          fallbackItem.title + " close up",
          "Ancient India Art",
        ],
      };
    }
  ...
  try {
    ...
    return {
      ...
      galleryPrompts: Array.isArray(parsed.galleryPrompts)
        ? parsed.galleryPrompts
        : [],
    };
  } catch (error) {
...
export const fetchDynamicSectionData = async (
  category: "TEMPLES" | "GODS" | "TEXTS",
  excludeNames: string[] = []
): Promise<any[]> => {
  ...
  if (!isGeminiConfigured()) {
    ...
    return newItems.map((item) => ({
      ...
      imageUrl: `https://image.pollinations.ai/prompt/${encodeURIComponent(
        (item.imageSearchTerm || item.title) + " " + BASE_IMAGE_PROMPT
      )}?width=600&height=400&nologo=true&seed=${item.id}`,
    }));
  }

  try {
    const excludeStr =
      excludeNames.length > 0 ? `Exclude: ${excludeNames.join(", ")}.` : "";
    const prompts = {
    ...
    const fallbackItems = dataMap[category].filter(
      (i) => !excludeNames.includes(i.title)
    );
...
export const playTextToSpeech = async (
  ...
  try {
    ...
    const validBuffer = await decodeAudioData(
      audioBytes,
      audioContext,
      24000,
      1
